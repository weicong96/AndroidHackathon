...
public class EventEndpoint {
    ...
    public Event get(@Named("eventID") Long eventID) throws NotFoundException {
        logger.info("Getting Event with ID: " + eventID);
        Event event = ofy().load().type(Event.class).id(eventID).now();
        List<UserEvent> userEvents = ofy().load().type(UserEvent.class).filter("eventRef", Ref.create(event)).filter("attending", true).list();
        if(userEvents != null){
            event.setAlreadyAttending(userEvents.size());
        }
    ...
    public Event insert(Event event) {
        ...
        // You should validate that event.eventID has not been set. If the ID type is not supported by the
    ...
    public Event update(@Named("eventID") Long eventID, Event event) throws NotFoundException {
    ...
    public void remove(@Named("eventID") Long eventID) throws NotFoundException {
        checkExists(eventID);
        ofy().delete().type(Event.class).id(eventID).now();
